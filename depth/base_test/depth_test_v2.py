"""
Depth Anything V2 Metric Indoor + RF-DETR VisionMessage 연동 코드

이 코드는 실내 방 이미지 1장을 입력으로 받아 Depth Anything V2 Metric Indoor 모델을 이용해
픽셀 단위 depth map(깊이 지도)을 생성하고, ai-perception 서비스에서 ZeroMQ REQ/REP 방식으로
전달받은 FlatBuffers 기반 VisionMessage의 객체 탐지 결과를 활용하여 객체 영역을 obstacle mask로 제외한다.

전체 처리 흐름은 다음과 같다.

1. shared/models/test_room.jpg 이미지를 로드한다.
2. ai-perception 서비스(tcp://ai-perception:5556 또는 tcp://127.0.0.1:5556)에 REQ 요청을 보낸다.
3. 응답으로 받은 FlatBuffers VisionMessage에서 objects 배열을 파싱한다.
   - objects 배열에는 id, label, x_min, y_min, x_max, y_max 정보가 포함된다.
   - bbox는 0~1 정규화 좌표가 아니라 절대 pixel 좌표이다.
   - label은 예: "의자|furniture_0.ply" 형식으로 들어올 수 있다.
4. VisionMessage의 객체 bbox를 기반으로 obstacle mask를 생성한다.
5. Depth Anything V2 Metric Indoor는 원본 이미지에 대해 depth map을 추론한다.
6. depth map을 3D point cloud로 변환한다.
7. obstacle mask 영역을 제외한 뒤 바닥 plane을 RANSAC 방식으로 추정한다.
8. 추정된 바닥 영역을 기반으로 바닥-벽 경계 후보선을 추출한다.
9. 결과를 JSON과 디버그 이미지로 저장한다.

저장 결과는 RESULT_NUMBER 값에 따라 다음 폴더에 생성된다.

예: RESULT_NUMBER = 1인 경우
C:/Users/khrha/Desktop/SWPJ_4/Depth/base_test/result_1/

생성 파일:
- result_1.json
  : 최종 depth 결과, VisionMessage 객체 정보, obstacle mask 정보, 바닥 plane 정보,
    바닥-벽 경계 후보선 정보를 저장한 JSON 파일

- result_1_depth.png
  : Depth Anything V2가 예측한 depth map 시각화 이미지

- result_1_vision_obstacle_mask.png
  : VisionMessage.objects의 bbox를 기반으로 생성한 객체 obstacle mask 시각화 이미지

- result_1_floor_mask.png
  : 객체 영역을 제외한 뒤 추정된 바닥 영역 시각화 이미지

- result_1_boundary_overlay.png
  : 추정된 바닥-벽 경계 후보선을 원본 이미지 위에 표시한 이미지

주의:
- 이 코드는 객체가 제거된 이미지를 Depth 모델에 입력하지 않는다.
- Depth 추론은 원본 이미지로 수행하고, 객체 bbox는 바닥/벽 구조 추정 단계에서 제외하는 방식으로 사용한다.
- FlatBuffers Python 생성 코드가 필요하며, shared/schema/InteriorPlatform.fbs를 flatc --python으로 변환해야 한다.
- Docker 내부에서 실행하면 VISION_ENDPOINT는 "tcp://ai-perception:5556"을 사용한다.
- Windows VSCode에서 직접 실행하고 docker-compose에서 5556 포트가 열려 있으면
  VISION_ENDPOINT를 "tcp://127.0.0.1:5556"으로 변경해야 한다.
"""
...
VISION_IMAGE_PATH_IN_SERVICE = "shared/models/thumb-1920-1174099.jpg"
