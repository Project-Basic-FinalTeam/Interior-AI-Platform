...
TOP_CATEGORY_CODE = "storage_accessories"
TOP_CATEGORY_NAME = "Storage accessories"
START_URL = "https://www.ikea.com/kr/en/cat/small-storage-organisers-st007/"

# Storage furniture ?덉뿉???쒖쇅???섏쐞 移댄뀒怨좊━
EXCLUDE_CATEGORY_NAMES = set()

# ?ъ슜???붿껌??留욎텣 ????대쫫
OUTPUT_LABEL = "Storage_accessories"
JSON_FILE_NAME = "ikea_Storage_accessories.json"
SKIPPED_FILE_NAME = "ikea_skipped_Storage_accessories.json"
IMAGE_FOLDER_NAME = "Storage_accessories"
IMAGE_FILE_PREFIX = "Storage accessories"
...
def parse_dimensions(text):
    ...
    def find_label_value(patterns):
        for ptn in patterns:
            m = re.search(ptn, s, re.IGNORECASE)
            if m:
                return convert_to_cm(float(m.group("val")), m.group("unit") or "cm")
        return None
    ...
    double = re.search(
        r"(?P<a>\d+(?:\.\d+)?)\s*[x×]\s*(?P<b>\d+(?:\.\d+)?)\s*(?P<unit>mm|cm|m)",
        s,
        re.IGNORECASE,
    )
    if double:
        unit = double.group("unit")
        a = convert_to_cm(float(double.group("a")), unit)
        b = convert_to_cm(float(double.group("b")), unit)
    ...
    if depth_y is None and height_z is not None:
        depth_y = height_z
...
def parse_product_detail(page, product_url, category_path, index):
    ...
    accept_cookie_if_present(page)
    expand_product_detail_sections(page)
    time.sleep(0.5)
    ...
    if width_x is None:
        missing_fields.append("width_x")
    if depth_y is None:
        missing_fields.append("depth_y")
    if height_z is None:
        missing_fields.append("height_z")
    if not image_url:
